...
const ColumnVisibilityMenu = ({ isOpen, visibility, onToggle, onClose, metadata }) => {

    if (!isOpen) return null;
    const columnKeys = Object.keys(visibility);

    return (
        <div className="absolute right-0 mt-2 w-64 bg-[#101438] rounded-lg shadow-xl z-50 p-4 border border-[#191d35]">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
                {columnKeys.map(key => {
                    const itemData = metadata.find(item => item.key === key);
                    const labelToDisplay = itemData ? itemData.label : key;
